 ...
 * ----------------------------------------------------------------
 * Installation (dans le HTML, AVANT ce fichier) :
 ...
 * Ces deux scripts exposent une variable globale `pdfMake` prête à l'emploi
 * (polices déjà configurées) - donc rien à faire ici, pas de require().
 *
 * Ce fichier montre :
...
function fullWidthRow(texte, nbColonnes, alignment) {
  const row = [{ text: parseHtmlToPdfmakeText(texte), colSpan: nbColonnes, alignment }];
  for (let i = 1; i < nbColonnes; i++) row.push({});
  return row;
}
...
async function genererEtTelecharger(
  ...
  // On fusionne toutes les catégories en une seule liste plate : preloadImages()
  // n'a pas besoin de savoir d'où vient chaque chemin, juste de les avoir tous.
  ...
  // 1. Convertir toutes les images en base64 en une fois
  const imagesBase64 = await preloadImages(cheminsUniques);

  // 2. Remplacer les chemins par les base64 correspondants dans une copie des données
  const leader = {
    ...leaderData,
  ...
  // 3. Construire et télécharger le PDF
  const docDefinition = buildDocDefinition(leader, pilotes);
  pdfMake.createPdf(docDefinition).download('liste-escadron.pdf');
}

// Pas de module.exports ici : ce fichier est chargé via une balise <script>
// classique dans le navigateur (pas de bundler), donc toutes les fonctions
